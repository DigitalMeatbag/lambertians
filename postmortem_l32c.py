...
def get_path(e):
    v = e.get('path') or e.get('url')
    if isinstance(v, list):
        return str(v)
    return v
rejected_paths = collections.Counter(get_path(e) for e in rejections)
...
for m in mem_stores:
    print(f"  t{m.get('turn_number')} path={m.get('path')} key={m.get('key')} content_len={len(str(m.get('content','')))} ")
print()
