...
- **`response_text` in `TURN_COMPLETE` is 800 chars; witness Narration panel shows 400 chars.** The witness sources text from docker log parsing, which uses the 400-char `response_excerpt` written to the log line. The event stream carries the full 800-char `response_text`, but the witness does not yet read from the event stream directly. Long Nemo turns are silently truncated in the Narration panel. Future improvement: refactor witness to tail `events.jsonl` directly instead of parsing docker logs.
